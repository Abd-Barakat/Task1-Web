    ...
    public partial class Edit_dialog : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)//for first time only
            {
                ...
                data = master.DB.extract_row(index);//return the specified two rows 
                ...
                master.GridView1.DataSource = temp_question_table;
                master.GridView1.DataBind();//bind temp_table to gridview
        ...
        private void Retrive_Data(DataRow[] rows, string QuestionType)
        {
            ...
            switch (QuestionType)
            {
                case "Slider":
                    ...
                    master.q = new Slider(question_text, question_order, Start, End, Start_Caption, End_Caption);//create slider object 
                    break;
                case "Smiley":
                    int Faces = (int)rows[1].ItemArray[1];//hold faces number from second row
                    master.q = new Smiley(question_text, question_order, Faces);//create smiley object 
                    break;
                case "Stars":
                    int Stars = (int)rows[1].ItemArray[1];//hold stars number from second row 
                    master.q = new Stars(question_text, question_order, Stars);//create star object 
                    break;
            }
        ...
        /// <summary>
        /// Grids the view1 update.
        /// </summary>
        /// <param name="q">The q.</param>
        private void GridView1_Update(Question q)
        {
            master.GridView1.Rows[0].Cells[0].Text = q.Question_text;
            master.GridView1.Rows[0].Cells[1].Text = q.Question_order.ToString();
            master.GridView1.Rows[0].Cells[2].Text = q.Question_type;
        ...
        protected void SaveButton_Click(object sender, EventArgs e)
        {
            ...
            if (master.Check(master.q.Current_values()))//call method check in Base class that check question's values if they are correct or not 
            {
                try
                {
                    ...
                    GridView1_Update(master.q);//update gridview with the new question
                    ...
                    master.Alert("Done!!");//notifying user that every thing is ok 
                    ScriptManager.RegisterClientScriptBlock(this, GetType(), "KEY", "setTimeout(function(){ Close(); },2000)", true);//call Show_Close() method written in javascript
            ...
            {
                master.q.Reset_values();//if current values are not correct
            }
